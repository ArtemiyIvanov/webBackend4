...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if ($errors['abilities']) {
        ...
        /* else {
            $messages[] = '<div class="error">Выберите допустимую способность.</div>';
        } */
    }
...
// 
    ...
    try {
        $stmt = $db->prepare("INSERT INTO task4 SET name = ?, email = ?, year = ?, sex = ?, limbs = ?, breakdown = ?, regeneration = ?, reaction = ?, resistance = ?, biography = ?, acquainted = ?");
        $stmt->execute([$_POST['fio'], $_POST['email'], intval($year) , $_POST['sex'], intval($limbs),  $ability_insert['breakdown'], $ability_insert['regeneration'], $ability_insert['reaction'], $ability_insert['resistance'], $_POST['field-name-2'], $_POST['acquainted']]);
    }
